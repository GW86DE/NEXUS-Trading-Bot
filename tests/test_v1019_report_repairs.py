...
def test_current_release_identity_and_diagnosis_match():
    assert (ROOT / "VERSION.txt").read_text(encoding="utf-8").strip() == "10.8.1-NEXUS"
    assert "10.8.1-RESTZEILEN-SCHNAPPSCHUSS-UND-KERZENCURSOR" in (ROOT / "RELEASE_BUILD.txt").read_text(encoding="utf-8")
    assert "TOOL_VERSION = '1.9.0'" in (ROOT / "NEXUS_10_Diagnose.py").read_text(encoding="utf-8")
    assert (ROOT / "NEXUS_10.8.1_Diagnose_Starten.sh").is_file()


def test_versioned_diagnosis_entry_is_bound_to_release():
    source = (ROOT / "NEXUS_10_8_1_Diagnose.py").read_text(encoding="utf-8")
    assert 'EXPECTED_VERSION = "10.8.1-NEXUS"' in source
