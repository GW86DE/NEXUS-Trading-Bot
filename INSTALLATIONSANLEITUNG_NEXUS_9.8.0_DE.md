# NEXUS 9.8.0 auf dem Raspberry Pi

Dieses Paket überarbeitet Kauf, Verkauf, Auftragsverfolgung und die WebUI. OKX-Verkäufe verwenden preisbegrenzte IOC-Orders; Teilverkäufe werden einzeln gebucht und der Rest wird erneut abgesichert. Es enthält den kompletten Bot. Der Updateweg übernimmt den aktuellen Zustand aus dem tatsächlich in beiden Diensten eingestellten Ordner.

## Mit dem selbstenthaltenden Installer

Speichere `NEXUS_9.8.0_Installieren.sh` im Ordner `/home/georg/Georg`. Starte ihn als Benutzer `georg` im Terminal:

```bash
bash "$HOME/Georg/NEXUS_9.8.0_Installieren.sh"
```

Der Starter prüft die eingebettete ZIP und entpackt nach `/home/georg/Georg/TradingBot_v9.8.0_NEXUS`. Der bestehende Updater prüft Abhängigkeiten und den isolierten Volltest, stoppt und sichert die bisherigen Dienste, übernimmt den Zustand, führt die vorhandene beleggestützte Reparatur aus und kontrolliert die neu gestarteten Dienste. Bei Systemschritten kann dein sudo-Passwort nötig sein.
...
```bash
bash "$HOME/Georg/NEXUS_9.8.0_Installieren.sh" --paket-pruefen
```

## Wenn du die ZIP verwendest

Entpacke `TradingBot_v9.8.0_NEXUS.zip` als neuen eigenen Ordner unter `/home/georg/Georg`. Starte danach:

```bash
bash "$HOME/Georg/TradingBot_v9.8.0_NEXUS/Nexus_Update.sh"
```
...
Am Ende muss `UPDATE UND STARTKONTROLLE ERFOLGREICH` erscheinen. Die WebUI zeigt 9.8.0 und im Menü „Handel“ statt getrennter Trades-/Positionen-Seiten. Lade die Seite nach dem Update einmal vollständig neu. Beide Dienste sollen den neuen Ordner verwenden:

```bash
systemctl show tradingbot-pi5.service tradingbot-webui.service --no-pager -p ActiveState -p SubState -p WorkingDirectory
```

Unter Handel → Klärung erscheinen neue, noch nicht abschließend verbuchte Aufträge. Ein alter ungeklärter Vorgang wird durch das Update nicht automatisch freigegeben.
...
```bash
python3 "$HOME/Georg/TradingBot_v9.8.0_NEXUS/Nexus_SUI_Diagnose.py"
```

Der Export enthält auch SUI-Aufträge, Einzelfills und Zustandsereignisse aus dem neuen Auftragsbuch.
